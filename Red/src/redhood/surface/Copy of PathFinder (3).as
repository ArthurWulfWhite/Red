package redhood.surface 
{
	...
	public class PathFinder 
	{
		...
		private var lowResGrid:Vector.<Vector.<Boolean>> = new Vector.<Vector.<Boolean>>(LOW_RES, true);
		
		private var skipList:SkipList;
		
		private var strengthenColor:ColorTransform = new ColorTransform ( 100, 100, 100 );
		
		private const PREDICTION_PENALTY:Number = 1.5;
		...
		private function processCell ( o:Object, x:int, y:int ):void
		{
			...
			if ( !lowResGrid[x][y] ) return;
			
			if ( o.y == y || o.x == x ) var diagonal:Boolean = false;
			else diagonal = true;
			
			if ( diagonal ) var dt:Number = SQRT2;
			else dt = 1;
			var t:Number = o.t + dt;
			...
			if ( !o2.done ) {
				var priority:Number = o2.t + minTime ( destX, destY, x, y ) * PREDICTION_PENALTY;
		...
		private function createGrids ( bdata:BitmapData ):void
		{
			...
			for ( var i:int = 0; i < LOW_RES; i ++ ) {
				var vec:Vector.<Boolean> = new Vector.<Boolean>(LOW_RES, true);
				lowResGrid[i] = vec;
				for ( var j:int = 0; j < LOW_RES; j ++ ) {
					vec[j] = ( lowRes.getPixel(i,j) >> 16 ) > 200;
				}
			}
